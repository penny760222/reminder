...
class Instance{
	...
	static function set($_class_key,$_path="",$_file=false,$_className=false){
		...
		//echo "path=".$tmp["full"]."<br>";
		if(!file_exists($tmp["full"])){
			echo "full=".$tmp["full"]."<br>";
			return false;
	...
	static function do_class_import($_className){
		...
		//echo "classname=$_className<br>";
		//echo "instance=".gettype($instance)."<br>";
		if(gettype($instance)=="NULL"){	//	找不到設定
			//throw new Exception ('class is not setting in instance'); //丟出錯誤
			return false;
		}
	...
	static function get($_class_key){ 
		...
		//echo "cc=$_class_key\n";
		//echo gettype($instance);
		//print_r($instance);
		//echo $instance["path"]."class.$_className.php";
